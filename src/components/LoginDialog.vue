...
<template>
  <v-dialog v-model="loginDialogVisible" max-width="500" persistent>
    <v-card rounded="xl">
      ...
      <v-card-text class="pt-0">
        <!-- Tabs -->
        <v-tabs v-model="state.isLogin" class="mb-md-4 mb-3" grow color="primary"
        ...
        <v-tabs-window v-model="state.isLogin">
          ...
          <v-tabs-window-item :value="true">
            <v-form class="mt-2">
              <v-text-field v-model="state.login.email" label="邮箱或用户名" type="email" variant="outlined"
                density="compact" />
              <v-text-field v-model="state.login.password" label="密码" type="password" variant="outlined"
                density="compact" @keyup.enter="onAuth" />
              <v-btn block color="error" class="mt-md-4 mt-0" size="default" rounded="pill" @click="onAuth">
                登录
              </v-btn>
            </v-form>
          </v-tabs-window-item>
          ...
          <v-tabs-window-item :value="false">
            <v-form class="mt-2">
              <v-text-field v-model="state.register.email" label="邮箱或用户名 (最少6位)" type="email" variant="outlined"
                density="compact" />
              <v-text-field v-model="state.register.password" label="密码" type="password" variant="outlined"
                density="compact" />
              <v-text-field v-model="state.register.password_repeat" label="重复密码" type="password" variant="outlined"
                density="compact" />
              <v-text-field v-model="state.register.invite_code" label="邀请码" variant="outlined" density="compact" />
              <v-btn block color="error" size="default" rounded="pill" @click="onPrepareRegister">
                注册
              </v-btn>
            </v-form>
          </v-tabs-window-item>
        ...
        <!-- Mobile extra links -->
        <div v-if="screenMode === 'phone'" class="mt-2">
          <v-list density="compact" class="pa-0">
            <v-list-item class="px-1 py-0">
              <v-list-item-subtitle style="word-break: break-all">
                <strong>备用地址：</strong>
                <a :href="store.configuration?.home_url" target="_blank" class="text-primary f14">
            ...
            <v-list-item class="px-1 py-0">
              <v-list-item-subtitle style="word-break: break-all">
                <strong>备用地址：</strong>
                <a :href="store.configuration?.domain_next" target="_blank" class="text-primary f14">
  ...
  <ForgotPassword ref="forgotPasswordRef" @open="loginDialogVisible = true" />
</template>
