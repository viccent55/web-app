...
<template>
  <div class="feeds-container" ref="feedsContainer" @scroll.passive="onScroll">
    <MasonryWall v-if="wallVisible" ref="masonryRef" :items="items" :min-columns="minColumns" :gap="internalGap"
      :scroll-container="feedsContainer" item-key="id" :ssr="false">
    ...
    <div v-if="!isLoadMore && items.length > 0" class="load-more-sentinel" v-intersect="{
      handler: onIntersect,
      options: {
