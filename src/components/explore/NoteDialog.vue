...
<template>
  <v-dialog v-model="noteDialogVisible" max-width="1200" height="100%" @after-enter="onOpenNoteDialog"
    :fullscreen="smAndDown" scrollable>
    <v-card :loading="state.loading">
      <v-card-title class="pt-0 px-3" v-if="smAndDown">
      ...
      <v-card-text class="pa-0">
        <v-row no-gutters>
          ...
          <v-col cols="12" md="5" lg="4">
            <div class="d-flex justify-space-between align-center mb-2 py-0 pr-4" v-if="!smAndDown">
              <AuthorHeader :loading="state.loading" :author="{
                ...state.data?.author,
                isFollow: state.data?.isFollow,
              }" @click-close="noteDialog.closeNoteDialog" @click-author="handle.clickAuthor"
                @click-follow="handle.clickFollow" />
              <v-btn color="primary" icon size="small" @click="noteDialog.closeNoteDialog">
            ...
            <div class="flex-grow-1 overflow-y-auto px-4" ref="note-dialog">
              ...
              <v-row dense v-if="screenMode === 'phone'">
                <v-col cols="12">
                  <v-toolbar class="d-flex justify-space-between align-center mb-2 py-0 rounded" density="compact">
                    <AuthorHeader :author="{
                      ...state.data?.author,
                      isFollow: state.data?.isFollow,
                    }" @click-close="noteDialog.closeNoteDialog" @click-author="handle.clickAuthor"
              ...
              <v-divider :thickness="1" class="my-3 border-opacity-25 text-grey" />

              <div>
                <div class="text-subtitle-2 mb-2" v-if="state.comments?.length">
      ...
      <v-card-actions class="border-t">
        <BottomAction ref="bottomActions" :action="state.data" :total="state.data?.comment_count"
          ...
          @click-share="handle.clickShare" @click-reply-to="handle.clickReplyTo" />
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
