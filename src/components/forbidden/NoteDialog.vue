...
<template>
  <v-dialog v-model="noteDialogVisible" max-width="1200" :persistent="screenMode === 'phone'" height="100%"
    @after-enter="onOpenNoteDialog" :fullscreen="smAndDown" scrollable>
    <v-card class="main-contain" :loading="loading">
      ...
      <v-card-text class="pa-0">
        <v-row no-gutters>
          ...
          <v-col cols="12" md="7" lg="8" class="d-flex align-center justify-center"
            ...
            <Video ref="swiperInstanceRef" v-if="state.data?.m3u8" :src="state.data?.m3u8"
              :height="smAndDown ? '300px' : 'calc(100vh - 120px)'" />
          </v-col>

          <!-- Right: Info & Comments -->
          <v-col cols="12" md="5" lg="4">
            <div class="d-flex justify-end mb-2 pt-2 pr-4" v-if="!smAndDown">
            ...
            <div class="flex-grow-1 overflow-y-auto px-4" ref="note-dialog">
              ...
              <v-divider :thickness="1" class="my-3 border-opacity-75" />
      ...
      <v-card-actions class="border-t">
        <BottomAction ref="bottomActions" :action="state.data" :total="state.data?.comment_count"
          @click-like="handle.clickLike" @click-star="handle.clickStar" @click-share="handle.clickShare" />
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
