...
const onDrawerSelect = (item: { province: string | null; city: string | null }) => {
...
<template>
  <v-container class="pa-0 h-100" fluid style="height: 100%">
    <v-card flat color="transparent">
      ...
      <v-card-title class="channel-bar-sticky px-0 ">
        <v-toolbar color="surface" density="compact">
          <v-row dense align="center">
            <v-col cols="4">
              <v-btn variant="text" class="px-2 ml-0 ml-md-2" @click="state.isDrawerOpen = true">
            ...
            <v-col cols="4" align-self="center">
              <div class="d-flex justify-center text-h6 text-primary font-weight-bold">
                精选推荐
              </div>
            </v-col>
            <v-col cols="4">
              <div class="d-flex align-center justify-end">
                <v-btn icon variant="text" @click="state.isFilterOpen = true">
        ...
        <!-- Category Tabs -->
        <v-tabs v-model="state.filter.cid" color="primary" class="category-tabs px-0 mb-2 px-md-2" density="compact"
          show-arrows @update:model-value="onCategoryChange">
          <v-tab v-for="item in displayMenu" :key="item.id" :value="item.id" class="px-0 custom-tab">
            {{ item.name }}
          </v-tab>
        </v-tabs>
      </v-card-title>
      ...
      <v-card-text class="px-3  ">
        <div ref="containerRef" class="hookup-wrapper">
          <v-row :dense="smAndDown">
            <v-col v-for="(item, index) in state.items" :key="index" cols="6" sm="4" md="4" lg="3" class="col-lg-1-5">
              <v-card tag="a" flat rounded="lg" class="hookup-card" :to="'/hookup/' + item.id"
                ...
                <Image :src="item?.cover" min-height="200px" :aspect-ratio="(180 / 280) * 1.5" cover />
                <div class="pa-2">
                  ...
                  <v-chip v-for="(tag, tIndex) in item?.tags" :key="tIndex" size="small" class="mr-1 mt-1 rounded-lg"
          ...
          <!-- Checkpoint Button -->
          <div class="checkpoint-btn" @click="onOpenCheckPoint">
            <div class="px-2 py-3 d-flex flex-column text-white font-weight-bold f12">
              <div>签到免</div>
              <div>定金</div>
            </div>
          </div>
        </div>
        <div v-if="!state.isNoMore && state.items.length > 0" class="load-more-sentinel" v-intersect="{
          handler: onIntersect,
          options: {
    ...
    <!-- Drawers & dialogs -->
    <DrawerArea v-model="state.isDrawerOpen" :areas="state.config?.areas" :hot-areas="state.config?.hot_areas"
      @select="onDrawerSelect" />
    <DrawerFilter v-model="state.isFilterOpen" :tags="state.config?.tags" @select="onTagSelect" />
