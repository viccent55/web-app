...
<template>
  <v-container fluid class="px-0 py-0">
    <div ref="pageWrapperRef" class="wrap-page">
      <div class="user-background" :style="{
        backgroundImage: `url(${store?.configuration?.member_center_background})`,
      }"></div>

      <div class="user-content-container">
        <div class="user-content mx-md-4">
          <UserInfo :user="userInfo" :loading="pending" @click-follow="clickFollow" @refresh="onInit" />
          <v-alert v-if="self && userInfo.status == 0" type="warning" class="mt-4" border="start">
            ⚠️ 未验证邮箱 {{ userInfo.email }} 验证后可订阅最新域名防止失联
            <v-btn small color="primary" :loading="loading" class="ml-2" @click="onVeryEmail">
              验证邮箱
            </v-btn>
          </v-alert>
          ...
          <v-row class="mt-2 px-3 px-md-0 w-100">
            <v-col cols="6">
              <v-card class="text-center pa-3" :height="smAndDown ? 140 : 200" rounded="xl"
                :color="config?.ai_clothes_background || '#965757'" @click="onOpenAiDialog('left')">
                <div class="text-center text-subtitle-1 text-md-h6">
                  {{ config?.ai_clothes_title || "AI脱衣" }}
                </div>
                <v-avatar :size="smAndDown ? 80 : 140" color="white">
            ...
            <v-col cols="6">
              <v-card class="text-center pa-3" :height="smAndDown ? 140 : 200" rounded="xl"
                :color="config?.girl_join_background || '#965757'" @click="onOpenAiDialog('right')">
                <div class="text-center text-subtitle-1 text-md-h6">
          ...
          <div class="menu-grid pa-3 mt-2">
            <div v-for="(item, i) in displayMenu" :key="i" class="menu-item" @click="onClickMenu(item)">
      ...
      <v-dialog v-model="isShowPupup" max-width="450">
        <v-card>
          <v-card-title></v-card-title>
          <v-card-text class="pb-0">
            <v-form>
              <v-text-field label="验证码" v-model="code" type="number" />
            </v-form>
          </v-card-text>
          <v-card-actions>
            <div class="d-flex justify-end ga-3 ma-2">
              <v-btn variant="elevated" @click="isShowPupup = false">
                取消
              </v-btn>
              <v-btn variant="elevated" @click="verifyEmail" color="primary">
                确认
              </v-btn>
            </div>
          </v-card-actions>
      ...
      <UserDialogAI ref="dialog-ai" @close="showChatWidget()"></UserDialogAI>
      <UserPersonalNote ref="popup-data" />
      <UserDialogInvites ref="inviteRef" :user-info="userInfo" />
